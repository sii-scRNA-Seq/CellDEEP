---
output: github_document
---

<!-- README.md is generated from README.Rmd. Please edit that file -->

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "man/figures/README-",
  out.width = "100%"
)
```

# CellDEEP

<!-- badges: start -->
<!-- badges: end -->

The goal of CellDEEP is to ...

## Installation
You can install the development version of CellDEEP like so:

Please download the package from github directly, then load the Rproject file.

``` {r,eval=FALSE}
#This is not working for now
devtools::install_github("sii-scRNA-Seq/CellDEEP")

```

## Example

Before using, don't forget library it:

```{r example}
library(CellDEEP)
```

To quickly run CellDEEP, just need to prepare the data, then run `FindMarker.CellDEEP`:

```{r cars, eval=FALSE}
#This is a test to see if the code is working, will reutrn 0 DE genes.
#For a actual test, please use data like: /datastore/Yiyi/datathon/Test_Dataset/covid_pDC_severeVShealthy.rds

pdc <- prepare_data(pDC, sample_id = "sample_id", group_id = "Worst_Clinical_Status", cluster_id = "initial_clustering", assay = "covid")

#Randomly pooling:
de.test <- FindMarker.CellDEEP(pdc, Pool = TRUE, ident.1 = "Healthy", ident.2 = "Severe", group.by = "group_id", n_cells = 2, assay = "RNA", pool_way = "random", cell_cutoff = 1)
```
There is an example of Kmean clustering:
```{r}
#K-mean pooling:
de.test <- FindMarker.CellDEEP(pdc, Pool = TRUE, ident.1 = "Healthy", ident.2 = "Severe", group.by = "group_id", n_cells = 6, cell_cutoff = 1, assay = "RNA", pool_way = "kmean")
```

It won't work as sample data has too limited cells to get sufficient Kmean clustering. Try it on a bigger data!

You'll still need to render `README.Rmd` regularly, to keep `README.md` up-to-date. `devtools::build_readme()` is handy for this.

You can also embed plots, for example:

In that case, don't forget to commit and push the resulting figure files, so they display on GitHub and CRAN.
